...
export class BookComponent implements AfterViewInit, OnInit {
  ...
  showModal(area:any, title:string): void {
    this.modalRef = this.modalService.create({
      ...
      nzOnOk: () => {
        ...
        this.firstStepForm.controls['username'].setValue(this.userService.getUsername());
  ...
  calculateTotalPayment(){
    ...
    if(this.userType == "Public User" && this.selectedEquipments.length > 0){
  ...
  constructor(private fb: UntypedFormBuilder, private bookService:BookService, private datePipe: DatePipe, private inventoryService:InventoryService, private modalService: NzModalService, private roomService:RoomService, private randService:RandomService, private refreshmentService:RefreshmentService, private userService:UserService, private router:Router){
  ...
  disabledDate:any;
    ...
    showDefaultDisabledDate(){
        this.disabledDate = (current: Date): boolean => {
          ...
          //Date is disabled for all weekdays in the case of public user
          if(this.userType == "Public User"){
  ...
  next(): void {
    if(this.firstStepForm.valid){
      if(this.current == 2){
        ...
        //In the case of public user, booking fee is required
        if(this.userType == "Public User"){
    ...
    if(this.allowBooking){
      ...
      Swal.fire("Warning","You have a pending booking! No additional booking is allowed!","warning");
    }
  }
  ...
  //Check any existing pending booking (Only one booking is allowed)
  checkExistingBooking(){
    this.bookService.checkExistingBooking(this.userService.getUsername()).subscribe(res=>{
      this.response = res;
      console.log(this.response);
      if(this.response){
  ...
  //Responsiveness for Steps Component
